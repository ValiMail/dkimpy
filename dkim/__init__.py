...
class ARC(DomainSigner):
  ...
  def sign(self, selector, domain, privkey, srv_id, include_headers=None,
    ...
    # consolidate AR headers & extract cv
    ar_headers = [res.strip() for [ar, res] in self.headers if ar == b'Authentication-Results']
    parsed_headers = [AuthenticationResultsHeader.parse('Authentication-Results: ' + res.decode('utf-8'))
                      for res in ar_headers]
    parsed_headers = [res for res in parsed_headers if res.authserv_id == srv_id]

    if len(parsed_headers) == 0:
      self.logger.debug("no AR headers found, chain terminated")
      return b''

    # TODO: merge headers    
    auth_results = ar_headers[0]
    
    arc_results = [res for res in parsed_headers[0].results if res.method == 'arc']
